...
// Unity Build Support - Use /unity directory for home and room views if it is available
const unityBuildExists = fs.existsSync(path.join(__dirname, '../../', 'public/unity/index.html'));
...
const views = {
  ...
  landing: unityBuildExists ? path.join(__dirname, '../../', 'public/unity/index.html') : path.join(__dirname, '../../', 'public/views/landing.html'),
...
app.get('/join/*', (req, res) => {
  if (hostCfg.authenticated) {
    if (unityBuildExists) {
      // Send meeting style room for /join/url's
      res.sendFile(views.meetingRoom);
    }
    else {
...
// general room selection for Unity builds
if (unityBuildExists) {
  app.get(['/*'], (req, res) => {
    if (hostCfg.authenticated) {
      res.sendFile(views.room);
...
io.on('connection', (socket) => {
  ...
  socket.on('globalmessage', (data) => {
    ...
    const sendTo = data.to_peer_id == 'all' ? socket.id : data.to_peer_id;

    roomList.forEach((value, key) => {
      roomList.get(value.id).broadCast(sendTo, 'globalmessage', data);
    });
  });
